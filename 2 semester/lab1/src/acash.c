...
void WithdrawMoney(ACCOUNT *account, const double sum_to_withdraw)
{
    ...
    if (strcmp(account->card_type, CREDIT) == 0)
    {
        ...
        {
            printf("Вы не можете снять деньги, потому что превысили "
...
int IsAccountClosed(const ACCOUNT *account)
{
    ...
    if (current_year == account->close_date.year && current_month > account->close_date.month)
        return 1;
    if (current_year == account->close_date.year && current_month == account->close_date.month && current_day >= account->close_date.day)
        return 1;
    return 0;
}
